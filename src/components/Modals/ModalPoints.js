...
const ModalPoints = ({ id, open, handleClose, data, ...rest }) => {
  ...
  return (
    ...
    >
      <DialogTitle id="alert-dialog-title">
        <span className="label">Ajouter des points</span>
      ...
      <DialogContent className="ta-modal-content-scroll">
        <Formik
          ...
          validationSchema={Yup.object().shape({
            code: Yup.string().max(255).required('Le code est requis.'),
            montant: Yup.string().max(255).required('Le montant  est requis.'),
            description: Yup.string().max(255).required('La description  est requis.'),
          })}
